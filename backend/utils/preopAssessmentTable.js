// New table for the "Preop Anesth" tab — a table WE own, alongside the
// other app_anes_* tables. Never touches existing hospital tables.
...
const TABLE_NAME = 'app_anes_preop_assessment';
...
async function ensurePreopAssessmentTable(dbType, conn) {
  if (ensured) return;
  ...
  if (dbType === 'mysql') {
    ...
    await conn.query(`
      CREATE TABLE IF NOT EXISTS ${TABLE_NAME} (
        ...
        INDEX app_anes_preop_assessment_an_idx (an)
      )
    `);
  } else {
    const columns = TEXT_COLUMNS.map((c) => `${c} text`).join(',\n      ');
    ...
    `);
    await conn.query(
      `CREATE INDEX IF NOT EXISTS app_anes_preop_assessment_an_idx ON ${TABLE_NAME} (an)`
    );
  }
...
module.exports = { TABLE_NAME, ALL_COLUMNS, ensurePreopAssessmentTable };
