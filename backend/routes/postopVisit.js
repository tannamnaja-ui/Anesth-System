...
const { TABLE_NAME, ALL_COLUMNS, ensurePostopVisitTable } = require('../utils/postopVisitTable');
...
// Latest saved postop visit record for this AN, if any.
router.get('/visit/:key', async (req, res) => {
  ...
  try {
    const row = await withConnection(dbType, dbConfig, async (conn) => {
      await ensurePostopVisitTable(dbType, conn);
