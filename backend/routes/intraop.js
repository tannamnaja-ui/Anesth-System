...
const { TABLE_NAME, ALL_COLUMNS, ensureIntraopTable } = require('../utils/intraopTable');
...
// Latest saved record for this case (AN, falling back to VN), if any.
router.get('/record/:key', async (req, res) => {
  ...
  try {
    const row = await withConnection(dbType, dbConfig, async (conn) => {
      await ensureIntraopTable(dbType, conn);
