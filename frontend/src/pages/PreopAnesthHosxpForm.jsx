...
// This is the "preop anesth HOSxP" tab — a separate table/record from the
// earlier "Preop Anesth" tab, by design (kept fully independent).
...
export default function PreopAnesthHosxpForm({ patient }) {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        const formRes = await api.getPreopAnesthHosxp(caseKey);
        if (cancelled) return;
        if (formRes.data) {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      const res = await api.savePreopAnesthHosxp(payload);
      setRecordId(res.id);
